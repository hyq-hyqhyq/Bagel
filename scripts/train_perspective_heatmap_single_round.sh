...
RUN_NAME=${RUN_NAME:-perspective_heatmap_single_round_score_reason_30k_4gpu}
RESULTS_DIR=${RESULTS_DIR:-/data/bagel/repo/Bagel/results/$RUN_NAME}
CHECKPOINT_DIR=${CHECKPOINT_DIR:-$RESULTS_DIR/checkpoints}
DATASET_CONFIG=${DATASET_CONFIG:-./data/configs/reason_heatmap_stage3_3000_heatmap_only.yaml}
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3} nohup torchrun \
  ...
  --master_addr=127.0.0.1 --master_port=${MASTER_PORT:-29505} \
  ...
  --split_gen_adapter_by_task True --gen_task_filter heatmap \
  ...
  --repair_mse_weight 1 --heatmap_mse_weight 10 \
  ...
  --max_num_tokens 27648 --max_num_tokens_per_sample 11520 \
