...
def load_lora_adapter(model, checkpoint_dir, logger):
...
def save_lora_adapter(fsdp_model, peft_model, checkpoint_dir):
    ...
    if dist.get_rank() == 0:
        ...
        adapter_state_dict = get_peft_model_state_dict(
            peft_model, state_dict=lora_state_dict
        )
        save_file(
            adapter_state_dict,
            os.path.join(checkpoint_dir, "adapter_model.safetensors"),
        )
        peft_model.peft_config["default"].save_pretrained(checkpoint_dir)
    dist.barrier()
...
def save_lora_checkpoint(
    ...
    save_path = os.path.join(checkpoint_dir, f"{train_step:07d}")
    logger.info(f"Saving LoRA checkpoint to {save_path}.")
    save_lora_adapter(fsdp_model, peft_model, save_path)
    ...
    with FSDP.state_dict_type(fsdp_model, StateDictType.LOCAL_STATE_DICT):
        ...
        elif dist.get_rank() < fsdp_config.num_shard:
            torch.save(optimizer.state_dict(), optimizer_path)
...
def run_validation(
    ...
    with FSDP.summon_full_params(fsdp_model, recurse=True, writeback=False):
...
def main():
    ...
    base_model_path = model_args.model_path
    ...
    if not resume_model_only and resume_from is not None:
        load_lora_adapter(model, resume_from, logger)
    if dist.get_rank() == 0:
